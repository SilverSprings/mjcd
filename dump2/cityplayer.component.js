...
export class PlayerComponent implements OnInit {
  ...
  //pInR hold the reference of the player's name, Id, and roomId 
  @Input() pInR:any;
  ...
  disableButton = true;
  handComplete = [false,false,false];
  playerSD = false;
  ...
  checkHandFirst() {
	...
	if (this.firstHandPlayed.cards.length == 3) {
		...
		this.handComplete[0] = true;
		if (handComplete.every( hC => hC)) {disableButton = false;} else {disableButton = true;}
	}
  ...
  checkHandLast() {  
	...
	if (this.lastHandPlayed.cards.length == 5) {
		...
		this.handComplete[2] = true;
		if (handComplete.every( hC => hC)) {disableButton = false;} else {disableButton = true;}	
	}
	if (this.lastHandPlayed.cards.length < 5) {
		this.handComplete[2] = false;	
	}
  }
  ...
  evaluate () {
	...
	this.handFinal["num"] = this.pInR.pId;
	this.handFinal["name"] = this.pInR.pName;	
