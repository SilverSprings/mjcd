...
export class ResultsComponent implements OnInit {
  ...
  */  
  ...
  busted:[false,false,false,false];
  ...
  ngOnInit() {
	...
	for (let _p = 0; _p < 4; _p++) {
		if (this.checkBustedHand(this.allHands[_p])) {
			this.allHands[_p].fhand.handValue = -1;
			this.allHands[_p].mhand.handValue = -1;
			this.allHands[_p].lhand.handValue = -1;
			this.busted[_p] = true;
		}
	}
  ...
  checkBustedHand(currentHand) {
	if (currentHand.fHand.handSubValue > currentHand.mHand.handSubValue) return true;
	if (currentHand.mHand.handSubValue > currentHand.lHand.handSubValue) return true;
	return false;
  }
